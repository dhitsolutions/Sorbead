...
{
    public partial class DebitNoteReport : Form
    {
        ...
        public DebitNoteReport(Master master, TabControl tabControl)
        {
        ...
        public void binddate()
        {
            try
            {
                ...
                dt = conn.getdataset("select Date1,TranType,AccountName,Amount,ShortNarration,dc,VoucherID from Ledger where isactive=1 and TranType='DEBIT NOTE' and  Date1>='" + Convert.ToDateTime(DTPFrom.Text).ToString(Master.dateformate) + "' and Date1<='" + Convert.ToDateTime(DTPTo.Text).ToString(Master.dateformate) + "' order by date1");
                if (dt.Rows.Count > 0)
                {
                    progressBar1.Increment(1);
        ...
        private void BtnViewReport_Click(object sender, EventArgs e)
        {
            if (userrights.Rows.Count > 0)
            {
                if (userrights.Rows[6]["v"].ToString() == "True")
                {
                    filelength = 1;
        ...
        private void btnprint_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Print popup = new Print("DebitReport");
                popup.ShowDialog();
                popup.Dispose();
            }
            catch
