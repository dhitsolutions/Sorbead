...
{
    public partial class SaleReturnApprovalForm : Form
    {
        ...
        public SaleReturnApprovalForm()
        {
            InitializeComponent();
        }

        public SaleReturnApprovalForm(Master master, TabControl tabControl)
        {
            InitializeComponent();
            this.master = master;
            ...
            if (dt1.Rows.Count > 0)
            {
                for (int i = 0; i < dt1.Rows.Count; i++)
                {
                    ...
                    if (drpitems.Text == "GroupName" || drpitems.Text == "Packing" || drpitems.Text == "Hsn_Sac_Code" || drpitems.Text == "itemnumber" || drpitems.Text == "ProductID")
                    {
                        ...
                        dt = conn.getdataset("select b.Bill_Date,b.billno,b.ClientID,bp.Productname,bp.batch,sum(bp.qty) as qty,sum(cast (bp.Aqty as float)) as aqty,sum(bp.free) as free,bp.Rate,sum(bp.Total) as total,bp.discountper,sum(bp.discountamt) as discountamt,sum(bp.cgstper + bp.sgstper + bp.igstper) as taxper,sum(bp.Tax) as tax,bp.addtaxper,sum(bp.addtax) as addtax,sum(bp.cess) as cess,sum(bp.Amount) as Amount,b.totalnet,bp.Productid,b.roudoff,b.apprweight,b.dispatchdetails,b.Delieveryat,b.fraight,b.vehicleno,b.grrrno,b.noofskids,b.remarks,b.totalcharges  from BillMaster b inner join BillProductMaster bp on b.billno=bp.billno where b.isactive=1 and bp.isactive=1 and bp.productid in (select Productid from productmaster where isactive=1 and (" + drpitems.Text + " like '%" + txtitems.Text + "%' )) and b.BillType='SR' and bp.BillType='SR' and bp.Status='N' and b.Bill_Date>='" + Convert.ToDateTime(DTPFrom.Text).ToString(Master.dateformate) + "' and b.Bill_Date<='" + Convert.ToDateTime(DTPTo.Text).ToString(Master.dateformate) + "' and b.billno='" + dt1.Rows[i]["billno"].ToString() + "' group by b.Bill_Date,b.billno,b.ClientID,bp.Productname,bp.batch,bp.Rate,bp.discountper,bp.addtaxper,bp.Productid,b.roudoff,b.totalnet,bp.cgstper,bp.sgstper,bp.igstper,b.apprweight,b.dispatchdetails,b.Delieveryat,b.fraight,b.vehicleno,b.grrrno,b.noofskids,b.remarks,b.totalcharges");
                    }
                    else if (drpitems.Text == "companyname")
                    {
                        ...
                        dt = conn.getdataset("select b.Bill_Date,b.billno,b.ClientID,bp.Productname,bp.batch,sum(bp.qty) as qty,sum(cast (bp.Aqty as float)) as aqty,sum(bp.free) as free,bp.Rate,sum(bp.Total) as total,bp.discountper,sum(bp.discountamt) as discountamt,sum(bp.cgstper + bp.sgstper + bp.igstper) as taxper,sum(bp.Tax) as tax,bp.addtaxper,sum(bp.addtax) as addtax,sum(bp.cess) as cess,sum(bp.Amount) as Amount,b.totalnet,bp.Productid,b.roudoff,b.apprweight,b.dispatchdetails,b.Delieveryat,b.fraight,b.vehicleno,b.grrrno,b.noofskids,b.remarks,b.totalcharges  from BillMaster b inner join BillProductMaster bp on b.billno=bp.billno where b.isactive=1 and bp.isactive=1 and bp.productid in (select productid from productmaster where isactive=1 and companyid in (select companyid from companymaster where isactive=1 and (companyname like '%" + txtitems.Text + "%')))  and b.BillType='SR' and bp.BillType='SR' and bp.Status='N' and b.Bill_Date>='" + Convert.ToDateTime(DTPFrom.Text).ToString(Master.dateformate) + "' and b.Bill_Date<='" + Convert.ToDateTime(DTPTo.Text).ToString(Master.dateformate) + "' and b.billno='" + dt1.Rows[i]["billno"].ToString() + "' group by b.Bill_Date,b.billno,b.ClientID,bp.Productname,bp.batch,bp.Rate,bp.discountper,bp.addtaxper,bp.Productid,b.roudoff,b.totalnet,bp.cgstper,bp.sgstper,bp.igstper,b.apprweight,b.dispatchdetails,b.Delieveryat,b.fraight,b.vehicleno,b.grrrno,b.noofskids,b.remarks,b.totalcharges");
                    }
                    else
                    {
                        dt = conn.getdataset("select b.Bill_Date,b.billno,b.ClientID,bp.Productname,bp.batch,sum(bp.qty) as qty,sum(cast (bp.Aqty as float)) as aqty,sum(bp.free) as free,bp.Rate,sum(bp.Total) as total,bp.discountper,sum(bp.discountamt) as discountamt,sum(bp.cgstper + bp.sgstper + bp.igstper) as taxper,sum(bp.Tax) as tax,bp.addtaxper,sum(bp.addtax) as addtax,sum(bp.cess) as cess,sum(bp.Amount) as Amount,b.totalnet,bp.Productid,b.roudoff,b.apprweight,b.dispatchdetails,b.Delieveryat,b.fraight,b.vehicleno,b.grrrno,b.noofskids,b.remarks,b.totalcharges  from BillMaster b inner join BillProductMaster bp on b.billno=bp.billno where b.isactive=1 and bp.isactive=1  and b.BillType='SR' and bp.BillType='SR' and bp.Status='N' and b.Bill_Date>='" + Convert.ToDateTime(DTPFrom.Text).ToString(Master.dateformate) + "' and b.Bill_Date<='" + Convert.ToDateTime(DTPTo.Text).ToString(Master.dateformate) + "' and b.billno='" + dt1.Rows[i]["billno"].ToString() + "' group by b.Bill_Date,b.billno,b.ClientID,bp.Productname,bp.batch,bp.Rate,bp.discountper,bp.addtaxper,bp.Productid,b.roudoff,b.totalnet,bp.cgstper,bp.sgstper,bp.igstper,b.apprweight,b.dispatchdetails,b.Delieveryat,b.fraight,b.vehicleno,b.grrrno,b.noofskids,b.remarks,b.totalcharges");
                    }
