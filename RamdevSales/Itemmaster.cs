...
namespace RamdevSales
{
    ...
    public partial class Itemmaster : Form
    {
        ...
        {
            con.Open();
            userrights = con1.getdataset("Select * from UserRights where isactive=1 and uid='" + Master.userid + "'");
        ...
        private void btnexport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("select p.Product_Name,c.companyname,p.GroupName,p.Unit,p.Altunit,p.Convfactor,p.Packing,p.IsBatch,p.Hsn_Sac_Code,p.isserial,p.cessper,p.cessamt,p.taxslab,pp.Batchno,pp.BasicPrice,pp.SalePrice,pp.MRP,pp.PurchasePrice,pp.Barcode,pp.OpStock,pp.ExpDt,pp.mfgdt,pp.Expdays,pp.SelfVal,pp.minsaleprice,pp.oploose,pp.opstockval,p.itemnumber,p.minstock,p.maxstock,p.reorderqty,pp.batchpartcode,pp.batchpacking,pp.batchPoNo from ProductMaster p inner join ProductPriceMaster pp on p.ProductID=pp.ProductID  inner join CompanyMaster c on p.CompanyID=c.CompanyID where p.isactive=1 and pp.isactive=1  group by p.Product_Name,c.companyname,p.GroupName,p.Unit,p.Altunit,p.Convfactor,p.Packing,p.IsBatch,p.Hsn_Sac_Code,pp.Batchno,pp.BasicPrice,pp.SalePrice,pp.MRP,pp.PurchasePrice,pp.Barcode,pp.OpStock,pp.ExpDt,pp.mfgdt,pp.Expdays,pp.SelfVal,pp.minsaleprice,pp.oploose,pp.opstockval,p.isserial,p.cessper,p.cessamt,p.taxslab,p.itemnumber,p.minstock,p.maxstock,p.reorderqty,pp.batchpartcode,pp.batchpacking,pp.batchPoNo", con);
