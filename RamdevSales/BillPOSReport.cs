...
{
    public partial class BillPOSReport : Form
    {
        SqlConnection constr = new SqlConnection(ConfigurationManager.ConnectionStrings["qry"].ToString());

        DataTable dt = new DataTable();
        public BillPOSReport()
        {
            InitializeComponent();

        }

        private void BillPOSReport_Load(object sender, EventArgs e)
        {
            try
            {
                this.StartPosition = FormStartPosition.Manual;
                this.Location = new Point(0, 0);
                BillingPOSPrintReport crystal = new BillingPOSPrintReport();
