...
{
    public partial class cdnur_GSTR2_ : Form
    {
        ...
        public cdnur_GSTR2_(Master master, TabControl tabControl)
        {
        ...
        public void binddata()
        {
            try
            {
                ...
                main = new DataTable();
                main.Columns.Add("Note/Voucher Number", typeof(string));
                main.Columns.Add("Note/Voucher date", typeof(string));
                ...
                main.Columns.Add("Note/Voucher Value", typeof(string));
                ...
                for (int i = 0; i < invoicedt.Rows.Count; i++)
                {
                    ...
                    if (ecomgst.Rows.Count > 0)
                    {
                        if (ecomgst.Rows[0]["TaxCalculation"].ToString() == "Tax Invoice")
                        {
                            ...
                            dr["Note/Voucher Number"] = invoicedt.Rows[i]["billno"].ToString(); ;
                            dr["Note/Voucher date"] = d; ;
                            ...
                            {
                                dr["Document Type"] = "D";
                                dr["Reason For Issuing Document"] = "Others";
                            }
                            ...
                            dr["Note/Voucher Value"] = invoicedt.Rows[i]["totalnet"].ToString();
        ...
        public void importexcel()
        {
            ...
            main.Rows.Add("Summary For CDNUR(6C)");
            main.Columns.Add("", typeof(string));
            main.Columns.Add("", typeof(string));
            main.Columns.Add("", typeof(string));
            main.Columns.Add("", typeof(string));
            ...
            main.Columns.Add("", typeof(string));
            main.Columns.Add("", typeof(string));
            main.Rows.Add("No of Notes/Vouchers", "", "No. of Invoices", "", "", "", "", "", "", "Total Note/Refund Voucher Value", "", "Total Taxable Value", "Total Integrated Tax Paid", "Total Central Tax Paid", "Total TState/UT Tax Paid", "Total Cess Paid", "", "Total Availed ITC Integrated Tax", "Total Availed ITC Central Tax", "Total Availed ITC State/UT Tax", "Total Availed ITC Cess");
            main.Rows.Add(totalgstno, totalbillno, "", totalnetvalue, "", "", "", "", "", "", totaltax, totalcess, "", "", "", "", "", "", "", "", "");
            main.Rows.Add("Note/Voucher Number", "Note/Voucher date", "Invoice/Advance Payment Voucher number", "Invoice/Advance Payment Voucher date", "Pre GST", "Document Type", "Reason For Issuing document", "Supply Type", "Invoice Type", "Note/Voucher Value", "Rate", "Taxable Value", "Integrated Tax Paid", "Central Tax Paid", "State/UT Tax Paid", "Cess Paid", "Eligibility For ITC", "Availed ITC Integrated Tax", "Availed ITC Central Tax", "Availed ITC State/UT Tax", "Availed ITC Cess");
            ...
            for (int i = 0; i < invoicedt.Rows.Count; i++)
            {
                ...
                if (ecomgst.Rows.Count > 0)
                {
                    if (ecomgst.Rows[0]["TaxCalculation"].ToString() == "Tax Invoice")
                    {
                        ...
                        if (ecomgst.Rows[0]["Region"].ToString() == "Local")
                        {
                            main.Rows.Add(invoicedt.Rows[i]["billno"].ToString(), d, invoicedt.Rows[i]["originalbillno"].ToString(), invoicedt.Rows[i]["originalbilldate"].ToString(), "N", documenttype, "", "Intra State", "Regular", invoicedt.Rows[i]["totalnet"].ToString(), rate, invoicedt.Rows[i]["totalt"].ToString(), invoicedt.Rows[i]["igstamt"].ToString(), invoicedt.Rows[i]["cgstamt"].ToString(), invoicedt.Rows[i]["sgstamt"].ToString(), invoicedt.Rows[i]["totalc"].ToString(), eligiable, invoicedt.Rows[i]["igstamt"].ToString(), invoicedt.Rows[i]["cgstamt"].ToString(), invoicedt.Rows[i]["sgstamt"].ToString(), invoicedt.Rows[i]["totalc"].ToString());
                            //totalbillno++;
                        }
                        else
                        {
                            main.Rows.Add(invoicedt.Rows[i]["billno"].ToString(), d, invoicedt.Rows[i]["originalbillno"].ToString(), invoicedt.Rows[i]["originalbilldate"].ToString(), "N", documenttype, "", "Inter State", "Regular", invoicedt.Rows[i]["totalnet"].ToString(), igst, invoicedt.Rows[i]["totalt"].ToString(), invoicedt.Rows[i]["igstamt"].ToString(), invoicedt.Rows[i]["cgstamt"].ToString(), invoicedt.Rows[i]["sgstamt"].ToString(), invoicedt.Rows[i]["totalc"].ToString(), eligiable, invoicedt.Rows[i]["igstamt"].ToString(), invoicedt.Rows[i]["cgstamt"].ToString(), invoicedt.Rows[i]["sgstamt"].ToString(), invoicedt.Rows[i]["totalc"].ToString());
                            // totalbillno++;
                        }
            ...
            //DISTINCT
            DataTable finalbill = conn.getdataset("select DISTINCT GstNo from (select c.GstNo,c.AccountName,c.State,c.statecode,b.originalbillno,b.originalbilldate,b.billno,b.Bill_Date,b.totalnet,bp.cgstper,bp.sgstper,bp.igstper,sum(bp.cgstamt) as cgstamt,sum(bp.sgstamt) as sgstamt,sum(bp.igdtamt) as igstamt,(bp.cgstamt+bp.sgstamt+bp.igdtamt+bp.addtax)as totaltax,b.totalcess,b.SaleType,sum(bp.Total-bp.discountamt) as totalt,sum(bp.cess) as totalc,b.BillType  from ClientMaster c inner join BillMaster b on c.ClientID=b.ClientID inner join BillProductMaster bp on b.billno=bp.billno  where c.isactive=1 and b.isactive=1 and bp.isactive=1 and b.BillType='PR' and bp.BillType='PR' and b.Bill_Date>='01-Apr-2018' and b.Bill_Date<='02-Apr-2018' and c.GstNo=''  group by bp.cgstamt,bp.sgstamt,bp.igdtamt,bp.addtax, c.GstNo,c.AccountName,c.State,c.statecode,b.originalbillno,b.originalbilldate,b.billno,b.Bill_Date,b.totalnet,bp.cgstper,bp.sgstper,bp.igstper,b.totaltax,b.totalcess,b.SaleType,b.BillType  union all  select c.GstNo,c.AccountName,c.State,c.statecode,b.originalbillno,b.originalbilldate,b.billno,b.Bill_Date,b.totalnet,case when bp.igst<= 0 then (tax/2) when bp.igst> 0 then 0 end as cgstper,case when bp.igst<= 0 then (tax/2) when bp.igst> 0 then 0 end as sgstper,case when bp.igst<= 0 then 0 when bp.igst> 0 then tax end as igstper,sum(case when bp.plusminus='-' then bp.cgst*-1 when bp.plusminus='+' then bp.cgst end) as cgstamt,sum(case when bp.plusminus='-' then bp.sgst*-1 when bp.plusminus='+' then bp.sgst end) as sgstamt,sum(case when bp.plusminus='-' then bp.igst*-1 when bp.plusminus='+' then bp.igst end) as igstamt,case when bp.plusminus='-' then (bp.cgst+bp.sgst+bp.igst+bp.addtaxamt)*-1 when bp.plusminus='+' then (bp.cgst+bp.sgst+bp.igst+bp.addtaxamt) end as totaltax,b.totalcess,b.SaleType,sum(case when bp.plusminus='-' then (bp.valueofexp)*-1 when bp.plusminus='+' then (bp.valueofexp) end) as totalt,0 as totalc,b.BillType  from ClientMaster c inner join BillMaster b on c.ClientID=b.ClientID inner join billchargesmaster bp on b.billno=bp.billno inner join BillSundry bs on bs.BillSundryID=bp.billsundryid and bs.OT3=1 and bs.isactive=1  where c.isactive=1 and b.isactive=1 and bp.isactive=1 and b.BillType='PR' and b.Bill_Date>='01-Apr-2018' and b.Bill_Date<='02-Apr-2018'and c.GstNo=''  group by bp.plusminus,bp.addtaxamt, c.GstNo,c.AccountName,c.State,c.statecode,b.originalbillno,b.originalbilldate,b.billno,b.Bill_Date,b.totalnet,cgst,bp.sgst,bp.igst,b.totaltax,b.totalcess,b.SaleType,bp.tax,b.BillType)entries");
            DataTable final = conn.getdataset("select DISTINCT billno from (select c.GstNo,c.AccountName,c.State,c.statecode,b.originalbillno,b.originalbilldate,b.billno,b.Bill_Date,b.totalnet,bp.cgstper,bp.sgstper,bp.igstper,sum(bp.cgstamt) as cgstamt,sum(bp.sgstamt) as sgstamt,sum(bp.igdtamt) as igstamt,(bp.cgstamt+bp.sgstamt+bp.igdtamt+bp.addtax)as totaltax,b.totalcess,b.SaleType,sum(bp.Total-bp.discountamt) as totalt,sum(bp.cess) as totalc,b.BillType  from ClientMaster c inner join BillMaster b on c.ClientID=b.ClientID inner join BillProductMaster bp on b.billno=bp.billno  where c.isactive=1 and b.isactive=1 and bp.isactive=1 and b.BillType='PR' and bp.BillType='PR' and b.Bill_Date>='01-Apr-2018' and b.Bill_Date<='02-Apr-2018' and c.GstNo=''  group by bp.cgstamt,bp.sgstamt,bp.igdtamt,bp.addtax, c.GstNo,c.AccountName,c.State,c.statecode,b.originalbillno,b.originalbilldate,b.billno,b.Bill_Date,b.totalnet,bp.cgstper,bp.sgstper,bp.igstper,b.totaltax,b.totalcess,b.SaleType,b.BillType  union all  select c.GstNo,c.AccountName,c.State,c.statecode,b.originalbillno,b.originalbilldate,b.billno,b.Bill_Date,b.totalnet,case when bp.igst<= 0 then (tax/2) when bp.igst> 0 then 0 end as cgstper,case when bp.igst<= 0 then (tax/2) when bp.igst> 0 then 0 end as sgstper,case when bp.igst<= 0 then 0 when bp.igst> 0 then tax end as igstper,sum(case when bp.plusminus='-' then bp.cgst*-1 when bp.plusminus='+' then bp.cgst end) as cgstamt,sum(case when bp.plusminus='-' then bp.sgst*-1 when bp.plusminus='+' then bp.sgst end) as sgstamt,sum(case when bp.plusminus='-' then bp.igst*-1 when bp.plusminus='+' then bp.igst end) as igstamt,case when bp.plusminus='-' then (bp.cgst+bp.sgst+bp.igst+bp.addtaxamt)*-1 when bp.plusminus='+' then (bp.cgst+bp.sgst+bp.igst+bp.addtaxamt) end as totaltax,b.totalcess,b.SaleType,sum(case when bp.plusminus='-' then (bp.valueofexp)*-1 when bp.plusminus='+' then (bp.valueofexp) end) as totalt,0 as totalc,b.BillType  from ClientMaster c inner join BillMaster b on c.ClientID=b.ClientID inner join billchargesmaster bp on b.billno=bp.billno inner join BillSundry bs on bs.BillSundryID=bp.billsundryid and bs.OT3=1 and bs.isactive=1  where c.isactive=1 and b.isactive=1 and bp.isactive=1 and b.BillType='PR' and b.Bill_Date>='01-Apr-2018' and b.Bill_Date<='02-Apr-2018'and c.GstNo=''  group by bp.plusminus,bp.addtaxamt, c.GstNo,c.AccountName,c.State,c.statecode,b.originalbillno,b.originalbilldate,b.billno,b.Bill_Date,b.totalnet,cgst,bp.sgst,bp.igst,b.totaltax,b.totalcess,b.SaleType,bp.tax,b.BillType)entries");
            totalbillno = Convert.ToString(final.Rows.Count);
            totalgstno = Convert.ToString(finalbill.Rows.Count);
            main.Rows[2][0] = totalgstno;
            main.Rows[2][2] = totalbillno;
