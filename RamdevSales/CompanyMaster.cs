    ...
    public partial class CompanyMaster : Form
    {
        ...
        private void CompanyMaster_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (userrights.Rows[25]["a"].ToString() == "False")
                    {
            ...
            try
            {
                ...
                else
                {
                    if (!string.IsNullOrEmpty(txtcompname.Text))
                    {
                        ...
                        SqlCommand cmdRecordExist = new SqlCommand("SELECT * FROM CompanyMaster WHERE companyname='" + txtcompname.Text + "'", con);
                        ...
                        {
                            MessageBox.Show(txtcompname.Text + " is already exist.");
                            return;
                        }
                        else
                        {
        ...
        private void listviewbind()
        {
            try
            {
                ...
                for (int i = 0; i <= dt.Rows.Count - 1; i++)
                {
                    ...
                    if (Convert.ToString(dt.Rows[i].ItemArray[2]) == "0")
                    {
                        LVclientadd.Items[i].SubItems.Add(string.Empty);
                    }
                    else
                    {
                        LVclientadd.Items[i].SubItems.Add(dt.Rows[i].ItemArray[2].ToString());
                    }
                    if (Convert.ToString(dt.Rows[i].ItemArray[3]) == "0")
                    {
                        LVclientadd.Items[i].SubItems.Add(string.Empty);
                    }
                    else
        ...
        {
            if (userrights.Rows[25]["u"].ToString() == "True")
            {
                open();
            }
            else
