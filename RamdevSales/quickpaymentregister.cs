    ...
    public partial class quickpaymentregister : Form
    {
        ...
        public void binddata()
        {
            try
            {
                ...
                DataTable dt1 = conn.getdataset("select * from Ledger where isactive=1 and TranType='Pmnt' and Date1>='" + Convert.ToDateTime(DTPFrom.Text).ToString(Master.dateformate) + "' and Date1<='" + Convert.ToDateTime(DTPTo.Text).ToString(Master.dateformate) + "'");
                if (dt1.Rows.Count > 0)
                {
                    progressBar1.Increment(1);
