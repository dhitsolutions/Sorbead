    ...
    public partial class ItemWiseStock : Form
    {
        ...
        private void _BindItemWiseStockList()
        {
            ...
            if (chkshowall.Checked == true)
            {
                ...
                string totalPurchase = conn.ExecuteScalar("select sum(pqty) from billproductmaster where bill_Run_Date < '" + DTPFrom.Text + "' and productid='" + proid + "' and Billtype='P' and isactive=1 and batch in(select batchno from productpricemaster where isactive=1 and productid='" + proid + "')");
                string totalPR = conn.ExecuteScalar("select sum(pqty) from billproductmaster where bill_Run_Date < '" + DTPFrom.Text + "' and productid='" + proid + "' and isactive=1 and Billtype='PR' and batch in(select batchno from productpricemaster where isactive=1 and productid='" + proid + "')");
                string totalSR = conn.ExecuteScalar("select sum(pqty) from billproductmaster where bill_Run_Date < '" + DTPFrom.Text + "' and productid='" + proid + "' and isactive=1 and Billtype='SR' and batch in(select batchno from productpricemaster where isactive=1 and productid='" + proid + "')");
                string totalPC = conn.ExecuteScalar("select sum(pqty) from saleorderproductmaster so inner join SaleOrderMaster s on s.billno=so.billno and s.isactive=1 where bill_Run_Date < '" + DTPFrom.Text + "' and productid='" + proid + "' and so.isactive=1 and so.Billtype='PC' and s.OrderStatus='Pending' and so.batch in (select batchno from productpricemaster where isactive=1 and productid='" + proid + "')");
                string totalSC = conn.ExecuteScalar("select sum(pqty) from saleorderproductmaster so inner join SaleOrderMaster s on s.billno=so.billno and s.isactive=1 where bill_Run_Date < '" + DTPFrom.Text + "' and productid='" + proid + "' and so.isactive=1 and so.Billtype='SC' and s.OrderStatus='Pending' and so.batch in (select batchno from productpricemaster where isactive=1 and productid='" + proid + "')");
                string totalSTI = conn.ExecuteScalar("select sum(pqty) from saleorderproductmaster so inner join SaleOrderMaster s on s.billno=so.billno and s.isactive=1 where bill_Run_Date < '" + DTPFrom.Text + "' and productid='" + proid + "' and so.isactive=1 and so.Billtype='STI' and s.OrderStatus='Pending' and so.batch in (select batchno from productpricemaster where isactive=1 and productid='" + proid + "')");
                string totalSTO = conn.ExecuteScalar("select sum(pqty) from saleorderproductmaster so inner join SaleOrderMaster s on s.billno=so.billno and s.isactive=1 where bill_Run_Date < '" + DTPFrom.Text + "' and productid='" + proid + "' and so.isactive=1 and so.Billtype='STO' and s.OrderStatus='Pending' and so.batch in (select batchno from productpricemaster where isactive=1 and productid='" + proid + "')");
                string totalSale = conn.ExecuteScalar("select sum(pqty) from billproductmaster where bill_Run_Date < '" + DTPFrom.Text + "' and productid='" + proid + "' and isactive=1 and Billtype='S' and batch in(select batchno from productpricemaster where isactive=1 and productid='" + proid + "')");
                string totalGIN = conn.ExecuteScalar("select sum(qty) from tblgoodissuereturnnoteitemmaster where Date < '" + DTPFrom.Text + "' and productid='" + proid + "' and isactive=1 and Billtype='GIN' and batch in(select batchno from productpricemaster where isactive=1 and productid='" + proid + "')");
                string totalGRN = conn.ExecuteScalar("select sum(qty) from tblgoodissuereturnnoteitemmaster where Date < '" + DTPFrom.Text + "' and productid='" + proid + "' and isactive=1 and Billtype='GRN' and batch in(select batchno from productpricemaster where isactive=1 and productid='" + proid + "')");
                if (totalPurchase == "" || totalPurchase == "NULL")
                {
                    totalPurchase = "0.00";
                }
                ...
                DataTable pos = conn.getdataset("select 'POS' as Billtype,b.BillDate as Bill_Run_Date,b.billno,b.totalnet as Rate,bp.qty as pqty,b.BillId as Bill_No, bp.Batchno as Batch,'' as PO_No,'' as username  from BillPOSMaster b inner join BillPOSProductMaster bp on b.billid=bp.billid where b.isactive=1 and bp.isactive=1 and b.BillDate>='" + Convert.ToDateTime(DTPFrom.Text).ToString(Master.dateformate) + "' and b.BillDate<='" + Convert.ToDateTime(DTPTo.Text).ToString(Master.dateformate) + "' and bp.itemname='" + txtitem.Text + "' and bp.batchno in(select batchno from productpricemaster where isactive=1 and productid='" + proid + "') order by BillDate asc");
                pos.Columns.Add("ClientID", typeof(String));
                //DataTable pos = conn.getdataset("select 'POS' as Billtype,BillDate as Bill_Run_Date,billno,totalnet as Rate,totalqty as pqty,BillId as Bill_No  from BillPOSMaster where isactive=1 and BillDate>='" + Convert.ToDateTime(DTPFrom.Text).ToString(Master.dateformate) + "' and BillDate<='" + Convert.ToDateTime(DTPTo.Text).ToString(Master.dateformate) + "' order by BillDate asc");
                pos = changedtclone(pos);

                // DataTable SPdt1 = conn.getdataset("select Billtype,Bill_Run_Date,billno,Rate,Pqty,Bill_No from SaleOrderProductMaster where isactive=1 and OrderStatus='Pending' and bill_Run_Date between '" + DTPFrom.Text + "' and '" + DTPTo.Text + "' and productid='" + proid + "' order by bill_Run_Date");
                DataTable SPdtSTI = conn.getdataset("select so.Billtype,so.Bill_Run_Date,so.billno,so.Rate,so.Pqty,so.Bill_No,so.ClientID,so.Batch as Batch,s.PO_No,u.username from SaleOrderProductMaster so inner join SaleOrderMaster s on so.BillNo=s.BillNo and s.isactive=1 inner join UserInfo u on u.UserId=s.Userid where so.isactive=1  and s.OrderStatus='Pending' and s.Billtype='STI' and so.bill_Run_Date between '" + DTPFrom.Text + "' and '" + DTPTo.Text + "' and so.productid='" + proid + "' and so.batch in (select batchno from productpricemaster where isactive=1 and productid='" + proid + "') order by so.bill_Run_Date");
                SPdtSTI = changedtclone(SPdtSTI);
                DataTable SPdtSTO = conn.getdataset("select so.Billtype,so.Bill_Run_Date,so.billno,so.Rate,so.Pqty,so.Bill_No,so.ClientID,so.Batch as Batch,s.PO_No,u.username from SaleOrderProductMaster so inner join SaleOrderMaster s on so.BillNo=s.BillNo and s.isactive=1 inner join UserInfo u on u.UserId=s.Userid where so.isactive=1 and s.isactive=1 and s.OrderStatus='Pending' and s.Billtype='STO' and so.bill_Run_Date between '" + DTPFrom.Text + "' and '" + DTPTo.Text + "' and so.productid='" + proid + "' and so.batch in (select batchno from productpricemaster where isactive=1 and productid='" + proid + "') order by so.bill_Run_Date");
                SPdtSTO = changedtclone(SPdtSTO);
                DataTable SPdt1 = conn.getdataset("select so.Billtype,so.Bill_Run_Date,so.billno,so.Rate,so.Pqty,so.Bill_No,so.ClientID,so.Batch as Batch,s.PO_No,u.username from SaleOrderProductMaster so inner join SaleOrderMaster s on so.BillNo=s.BillNo and s.isactive=1 inner join UserInfo u on u.UserId=s.Userid where so.isactive=1 and s.isactive=1 and s.OrderStatus='Pending' and s.Billtype='PC' and so.bill_Run_Date between '" + DTPFrom.Text + "' and '" + DTPTo.Text + "' and so.productid='" + proid + "' and so.batch in (select batchno from productpricemaster where isactive=1 and productid='" + proid + "') order by so.bill_Run_Date");
                SPdt1 = changedtclone(SPdt1);
                DataTable SPdt2 = conn.getdataset("select so.Billtype,so.Bill_Run_Date,so.billno,so.Rate,so.Pqty,so.Bill_No,so.ClientID,so.Batch as Batch,s.PO_No,u.username from SaleOrderProductMaster so inner join SaleOrderMaster s on so.BillNo=s.BillNo and s.isactive=1 inner join UserInfo u on u.UserId=s.Userid where so.isactive=1  and s.OrderStatus='Pending' and s.Billtype='SC' and so.bill_Run_Date between '" + DTPFrom.Text + "' and '" + DTPTo.Text + "' and so.productid='" + proid + "' and so.batch in (select batchno from productpricemaster where isactive=1 and productid='" + proid + "') order by so.bill_Run_Date");
                SPdt2 = changedtclone(SPdt2);
                DataTable SPdtsr = conn.getdataset("select so.Billtype,so.Bill_Run_Date,so.billno,so.Rate,so.Pqty,so.Bill_No,so.ClientID,so.Batch as Batch,s.PO_No,u.username from billproductmaster so inner join BillMaster s on so.BillNo=s.BillNo and s.isactive=1 inner join UserInfo u on u.UserId=s.Userid where so.isactive=1 and s.Billtype='SR' and so.bill_Run_Date between '" + DTPFrom.Text + "' and '" + DTPTo.Text + "' and so.productid='" + proid + "' and batch in (select batchno from productpricemaster where isactive=1 and productid='" + proid + "') order by so.bill_Run_Date");
                SPdtsr = changedtclone(SPdtsr);
                DataTable SPdtpr = conn.getdataset("select so.Billtype,so.Bill_Run_Date,so.billno,so.Rate,so.Pqty,so.Bill_No,so.ClientID,so.Batch as Batch,s.PO_No,u.username from billproductmaster so inner join BillMaster s on so.BillNo=s.BillNo and s.isactive=1 inner join UserInfo u on u.UserId=s.Userid where so.isactive=1 and s.Billtype='PR' and so.bill_Run_Date between '" + DTPFrom.Text + "' and '" + DTPTo.Text + "' and so.productid='" + proid + "' and batch in (select batchno from productpricemaster where isactive=1 and productid='" + proid + "') order by so.bill_Run_Date");
                SPdtpr = changedtclone(SPdtpr);
                DataTable SPdtp = conn.getdataset("select so.Billtype,so.Bill_Run_Date,so.billno,so.Rate,so.Pqty,so.Bill_No,so.ClientID,so.Batch as Batch,s.PO_No,u.username from billproductmaster so inner join BillMaster s on so.BillNo=s.BillNo and s.isactive=1 inner join UserInfo u on u.UserId=s.Userid where so.isactive=1 and s.Billtype='P' and so.bill_Run_Date between '" + DTPFrom.Text + "' and '" + DTPTo.Text + "' and so.productid='" + proid + "' and batch in (select batchno from productpricemaster where isactive=1 and productid='" + proid + "') order by so.bill_Run_Date");
                SPdtp = changedtclone(SPdtp);
                //   DataTable SPdtGIN = conn.getdataset("select Billtype,Date,billno,0 as Rate,qty as Pqty,0 as Bill_No,ClientID from tblgoodissuereturnnoteitemmaster where isactive=1 and billtype='GIN' and Date between '" + DTPFrom.Text + "' and '" + DTPTo.Text + "' and productid='" + proid + "' order by Date");
                //   SPdtGIN = changedtclone(SPdtGIN);
                //    DataTable SPdtGRN = conn.getdataset("select Billtype,Date,billno,0 as Rate,qty as Pqty,0 as Bill_No,ClientID from tblgoodissuereturnnoteitemmaster where isactive=1 and billtype='GRN' and Date between '" + DTPFrom.Text + "' and '" + DTPTo.Text + "' and productid='" + proid + "' order by Date");
                //     SPdtGRN = changedtclone(SPdtGRN);
                DataTable SPdt = conn.getdataset("select so.Billtype,so.Bill_Run_Date,so.billno,so.Rate,so.Pqty,so.Bill_No,so.ClientID,so.Batch as Batch,s.PO_No,u.username from billproductmaster so inner join BillMaster s on so.BillNo=s.BillNo and s.isactive=1 inner join UserInfo u on u.UserId=s.Userid where so.isactive=1 and s.Billtype='S' and so.bill_Run_Date between '" + DTPFrom.Text + "' and '" + DTPTo.Text + "' and productid='" + proid + "' and so.batch in (select batchno from productpricemaster where isactive=1 and productid='" + proid + "') order by so.bill_Run_Date");
